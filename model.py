...
while True: 
    ...
    ms=np.zeros((480,640,3))
    msh=np.expand_dims(mask,0)
    ms[:,:,0]=mask
    ms[:,:,1]=mask
    ms[:,:,2]=mask
    mask=ms
    ...
    img=mask 
    #cv2.imshow('Video Face Detection', mask) 
    #height , width , layers =  img.shape
    hhh=mask
    new_h=128
    new_w=128
    frame = cv2.resize(mask, (new_w, new_h))
 ...
 #   str=model.predict_classes()
    ...
    if prob[0][stri]>=0.90:
        ...
        if stri==1:
            cv2.putText(hhh,"DOWN",(0,100),font,2,(255,0,0),1,cv2.LINE_AA) #255,255,2550,0
        elif stri==2:
            cv2.putText(hhh,"FIST",(0,100),font,2,(255,0,0),1,cv2.LINE_AA)
        elif stri==3:
            cv2.putText(hhh,"FIST_MOVED",(0,100),font,2,(255,0,0),1,cv2.LINE_AA)
        elif stri==4:
            cv2.putText(hhh,"INDEX",(0,100),font,2,(255,0,0),1,cv2.LINE_AA)
        elif stri==5:
            cv2.putText(hhh,"LETTER L",(0,100),font,2,(255,0,0),1,cv2.LINE_AA)
        elif stri==6:
            cv2.putText(hhh,"OK",(0,100),font,2,(255,0,0),1,cv2.LINE_AA)
        elif stri==7:
            cv2.putText(hhh,"PALM",(0,100),font,2,(255,0,0),1,cv2.LINE_AA)
        elif stri==8:
            cv2.putText(hhh,"PALM MOVED",(0,100),font,2,(255,0,0),1,cv2.LINE_AA)
        elif stri==9:
            cv2.putText(hhh,"THUMB",(0,100),font,2,(255,0,0),1,cv2.LINE_AA)
        elif stri==0 :
            cv2.putText(hhh,"C",(0,100),font,2,(255,0,0),1,cv2.LINE_AA)
            proba=np.zeros((20,1))
        else:
            cv2.putText(hhh,"NOT RECOGNISED",(0,100),font,2,(255,0,0),1,cv2.LINE_AA)
